    ...
    def forward(self, g, feats):
        ...
        with g.local_scope():
            ...
            for ntype in g.srctypes:
                g.srcnodes[ntype].data['c'] = self.content_aggs[ntype](feats[ntype])  # (N_i, d_hid)

            # 2.同类型邻居聚集
            neighbor_embeds = {}
            for dt in g.dsttypes:
                if g.num_dst_nodes(dt) == 0:
                    continue
                tmp = []
                for st in g.srctypes:
                    # dt类型所有顶点的st类型邻居个数必须全部相同
                    g.multi_update_all({f'{st}-{dt}': (fn.copy_u('c', 'm'), stack_reducer)}, 'sum')
                    tmp.append(self.neighbor_aggs[st](g.dstnodes[dt].data.pop('nc')))
                neighbor_embeds[dt] = torch.stack(tmp)  # (A, N_dt, d_hid)

            # 3.类型组合
            content_embed = g.srcnodes[self.predict_ntype].data['c']
            if g.is_block:
                content_embed = content_embed[:g.num_dst_nodes(self.predict_ntype)]
            out = self.predict(self.comb(content_embed, neighbor_embeds[self.predict_ntype]))
            return out
