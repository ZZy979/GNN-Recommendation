...
def accuracy(predict, labels):
    ...
    :return: float 准确率
    """
    return torch.sum(predict == labels).item() * 1.0 / len(labels)
...
@torch.no_grad()
def evaluate(model, loader, g, labels, num_classes, predict_ntype, train_idx, val_idx, test_idx):
    """评估模型性能
...
def calc_metrics(logits, labels, train_idx, val_idx, test_idx):
    predict = logits.detach().cpu().argmax(dim=1)
    labels = labels.cpu()
    train_acc = accuracy(predict[train_idx], labels[train_idx])
    val_acc = accuracy(predict[val_idx], labels[val_idx])
    test_acc = accuracy(predict[test_idx], labels[test_idx])
