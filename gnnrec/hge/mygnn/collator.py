...
class PositiveSampleCollator(Collator):
    ...
    def __init__(self, g, block_sampler, pos, ntype=None):
        ...
        :param g: DGLGraph 原图
        :param block_sampler: BlockSampler 邻居采样器，层数必须为1
        :param pos: tensor(N, T_pos) 每个顶点的正样本id
        :param ntype: str, optional 如果g是异构图则需要指定目标顶点类型
        """
        if block_sampler.num_layers != 1:
            raise ValueError('邻居采样器的层数必须为1')
    ...
    def collate(self, items):
        ...
        block = self.block_sampler.sample_blocks(self.g, output_nodes)[0]
        return block
